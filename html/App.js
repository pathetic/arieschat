...
window.APP = {
  ...
  watch: {
	...
	messagesGS() {
      ...
      const messagesObj = this.$refs.messagesGS;
      this.$nextTick(() => {
        messagesObj.scrollTop = messagesObj.scrollHeight;
      });
    },
  ...
  methods: {
    ...
    ON_MESSAGEGS({ message }) {
      this.messagesGS.push(message);
    },
    ON_CLEAR() {
      this.messages = [];
	    this.messagesP = [];
    ...
    send(e) {
      ...
      if(msg !== '') {
        if (channel == 'global') {
          ...
          this.oldMessagesIndex = -1;
          this.hideInput();
        }else if (channel == 'gangs') {
          $.post('http://chat/chatResultGS', JSON.stringify({
            message: getEmojiChars(msg),
          }));
          this.oldMessages.unshift(msg);
          msg = '';
